...
当前版本为 0.4.4，安装向导、卸载显示名和开始菜单已统一为“FlexHMI 桌面版”，保留旧安装标识与数据路径。新版的实际安装、步骤控制、历史恢复、评估记录、授权与完整卸载验证见 [0.4.4 发布记录](releases/0.4.4.md)。下文也保留了旧版构建与验收的历史说明。
...
- **运行画面**：打开当前已保存工程。
- **全屏运行**：可选的全屏展示方式，隐藏编辑器。Alt+F4 关闭浏览器窗口。
...
安装包未签名。测试机验证不等于客户现场验证；Windows 10 / 11 x64 尚未直接测试；真实 PLC 和串口设备连接可按具体项目另行验证。
