...
| rule.upsert | rule | 新增/合并控制规则；新增无效引用拒绝，已有依赖删除时停用并报告影响 |
...
`routeMode: "return"` 请求外侧回流通道，并从 `page.optimize` 的前向列排序中排除。其余连接存在环时先压缩为分组再按确定性顺序展开，保留原始 from/to 语义，给回流边设置派生 `layoutRole`。派生值不作为下次排序的用户意图；未显式标注回流时需核对算法推断。分支按已排列上游的行序排序；不会把未连接的标题、数据卡片强行排入设备列。
...
`GET /industry/evaluations` 可检索内置 AI 与外部 Agent 已完成的评估，包含没有工程改动的报告。返回摘要、下一页游标和损坏记录提示；`GET /industry/evaluations/:id` 读取引用与采样详情。详见 [行业评估](INDUSTRY.md)。记录是当时的证据，保存不延长观测有效期。

## 新增引用校验与删除关联

当前源码在自动清理之前检查新增或更改的引用。变量绑定、设备详情、连接端点、规则与步骤流程的变量，以及资料 ID/版本必须能在最终工程中找到；错误返回 `invalid-reference`，消息包含画面、对象和字段位置。设备 ID 不能作为变量 ID 使用。同一计划中允许先引用、后创建，最终对象存在即可。

已有组件保留的变量引用因删除而失效时，仍会清理绑定；已有连接的端点被删除时，仍会移除管线；已有规则或流程的依赖被删除或资料版本更新时，仍会停用，并展示影响。已停用的历史配置可以保留供修复。加载工程使用该保存工程作为关联基线，历史恢复使用原快照，不误用当前工程的对象关系。新工程不会继承同名旧对象的清理资格。

AI 工作台中，编辑结构化计划后必须重新检查。旧预览、错误 JSON 或编辑前发出的结构化计划检查响应不能重新启用“应用此计划”。此行为与引用校验尚未进入已发布的 0.4.3 安装包。详见 [本轮验收](reference-integrity/README.md)。

这些检查证明引用存在且预览对应当前文本，不能证明一个存在的变量在工艺语义上绑定正确，也不替代真实模型完整生成、行业资料质量和控制执行验收。

多分支与显式回流修复见 [验收记录](complex-layout-verification/README.md)。本轮尚未进入 0.4.3 安装包；复杂路径自动分离仍待完成。

独立管线过近或共线时，路由器可在有限搜索内平移中间通道，并给出 `channel-adjusted` 诊断。设备、端口和 from/to 保持不变；不可分离时继续给出 `route-overlap`，跨线仍不表示连通。重复优化稳定。见 [通道分离验收](channel-verification/README.md)，尚未纳入 0.4.3。
