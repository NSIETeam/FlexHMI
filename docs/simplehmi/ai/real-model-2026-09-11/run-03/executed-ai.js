...
// The job's AbortSignal owns the deadline; native fetch has a separate five-minute header limit.
const modelHttp = require('axios').create({adapter: 'http', timeout: 0, proxy: false, maxRedirects: 0, maxContentLength: 2000000, responseType: 'text', transformResponse: [text => text], validateStatus: () => true});
...
function normalizeConfig(input) {
  ...
  const timeoutSeconds = Number(input.timeoutSeconds ?? 120);
  if (!Number.isInteger(timeoutSeconds) || timeoutSeconds < 30 || timeoutSeconds > 900) throw Error('生成等待时间应为 30–900 秒的整数');
  return {provider: input.provider, baseUrl: url.href.replace(/\/+$/, ''), model: input.model.trim(), jsonMode: input.jsonMode !== false, timeoutSeconds};
}
...
async function completion(config, key, messages, signal) {
  ...
  let response;
  try {
    response = await modelHttp.post(url, body, {signal,
      headers: {'Content-Type': 'application/json', ...(key ? {Authorization: 'Bearer ' + key} : {})}});
  } catch (e) { if (signal.aborted) throw e; if (e.code === 'ERR_BAD_RESPONSE' && e.message.includes('maxContentLength')) throw Error('模型响应超过 2 MB 限制'); throw Error('无法连接模型服务，请检查地址、网络和证书（重定向不被接受）'); }
  if (response.status >= 300 && response.status < 400) throw Error('模型服务返回重定向，请填写最终服务地址；凭据未转发');
  if (response.status < 200 || response.status >= 300) throw Error(`模型服务返回 HTTP ${response.status}；${response.status >= 500 ? '服务暂时不可用，请稍后重试' : '请检查模型名、凭据或额度'}`);
  let data;
  try { data = JSON.parse(response.data); } catch { throw Error('模型服务返回了无效响应'); }
  if (ollama) {
...
function createAi({dir, getProject, getValues=()=>({}), digest, previewPlan, validatePlan, audit, complete = completion, timeoutMs}) {
  ...
  const view = job => ({id: job.id, status: job.status, stage: job.stage, attempt: job.attempt, createdAt: job.createdAt, elapsedMs: Math.max(0,(job.finishedAt || Date.now())-job.createdAt), timeoutSeconds: job.timeoutMs / 1000, ...(job.plan ? {plan: job.plan} : {}), ...(job.report ? {report: job.report} : {}), ...(job.evaluationId ? {evaluationId:job.evaluationId} : {}), ...(job.error ? {error: job.error} : {})});
  ...
  function cancel(id) {
    ...
    if (job.status === 'running') { job.status = 'cancelled'; job.finishedAt = Date.now(); job.stage = '已取消，未修改工程'; job.controller.abort(); audit({event: 'ai-cancelled', jobId: id}); }
    return view(job);
  }
  function start(input) {
    ...
    const waitBudget = Math.min(timeoutMs ?? selected.timeoutSeconds * 1000, isAssessment ? Math.max(0, evidence.capturedAt + 180000 - Date.now()) : Infinity);
    const job = {timeoutMs: waitBudget, id: 'gen_' + crypto.randomUUID().replaceAll('-', ''), status: 'running', stage: '模型正在生成工程计划', attempt: 1, createdAt: Date.now(), controller: new AbortController()};
    ...
    const timer = setTimeout(() => { job.timedOut = true; job.controller.abort(); }, job.timeoutMs); timer.unref?.();
    job.done = (async () => {
      try {
