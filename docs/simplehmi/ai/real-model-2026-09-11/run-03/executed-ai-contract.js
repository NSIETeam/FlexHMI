...
const machineAction=object({tagId:id,value:num,min:num,max:num});
const transition=object({to:id,priority:{type:'integer',minimum:0,maximum:99},afterMs:{type:'number',minimum:0,maximum:86400000},holdMs:{type:'number',minimum:0,maximum:60000},conditions:list(guard,16)});
const machineState=object({id,name:string,timeoutMs:{type:'number',minimum:0,maximum:86400000},actions:list(machineAction,16),transitions:list(transition,20)});
const machine=object({id,name:string,enabled:bool,initialState:id,maxAgeMs:{type:'number',minimum:500,maximum:60000},minIntervalMs:{type:'number',minimum:1000,maximum:3600000},guards:list(guard,16),states:list(machineState,30),evidence:list(citation,16)},['id','name','enabled','initialState','maxAgeMs','minIntervalMs','guards','states']);
const project=object({schemaVersion:{const:1},id,name:{type:'string',minLength:1,maxLength:100},system:object({mode},undefined,true),control:object({rules:list(ref('rule'),50),machines:list(ref('machine'),20)},['rules']),knowledge:list(ref('knowledge'),100),simulation:{enum:['water-transfer','waste-to-energy']},activePageId:id,devices:list(ref('device'),20),pages:{...list(ref('page'),30),minItems:1},customSymbols:list(ref('asset'),16)},['schemaVersion','id','name','devices','pages'],true);
const patch=name=>({type:'object',properties:{...({tag,device,component,connection,page,asset,rule,machine}[name].properties)},required:['id'],additionalProperties:true});
...
const variants=[op('project.create',{project:ref('project')}),op('project.load',{id,expectedSavedRevision:savedRevision}),op('project.delete',{id,expectedSavedRevision:savedRevision}),op('project.restore',{archiveId:id,expectedSavedRevision:savedRevision}),op('project.revert',{planId:id}),op('project.configure',{name:string,mode},[]),
 ...
 op('knowledge.upsert',{entry:ref('knowledge')}),op('knowledge.delete',{id}),op('rule.upsert',{rule:patch('rule')}),op('rule.delete',{id}),op('machine.upsert',{machine:patch('machine')}),op('machine.delete',{id}),op('page.optimize',{pageId:id}),op('project.optimize',{})];
const planSchema = {$schema:'http://json-schema.org/draft-07/schema#',title:'FlexHMI engineering operation plan v1',...object({summary:{type:'string',minLength:1,maxLength:500},operations:{type:'array',minItems:1,maxItems:500,items:{oneOf:variants}}}),definitions:{tag,device,component,connection,page,asset,project,rule,machine,knowledge}};
...
步骤流程通过 machine.upsert/delete 创建。initialState 指向起始步骤，每个 state 包含 id/name/timeoutMs/actions/transitions。actions 对该流程所有输出逐一声明 tagId/value/min/max；不同步骤必须声明同一组输出，不能与启用规则或其他流程占用同一输出。启动时真实输出必须与起始步骤目标一致，否则需先人工核对。transitions 包含 to、priority（同一步内唯一，数字越小越优先）、afterMs、holdMs、conditions（全部比较条件满足），无条件跳转至少等待1000毫秒。流程 minIntervalMs 至少1000；timeoutMs=0关闭该步超时，正值到时中止自动写入。没有跳转的步骤为终点并保持监测。每次切换只在逐项写入回读完成后推进，多个输出写入不是原子事务，失败或接管不回滚已发出的设备指令。guards 为流程持续运行允许条件，失效中止自动写入，不自动选择所谓安全输出。states 最多30，全部步骤需从起点可达；资料 evidence 与阈值规则采用相同验证。不生成无限零延时环。
用户没提供真实设备参数时使用 sim，绝不猜真实 PLC 地址。添加 ModbusTCP 会重启通信，需在 summary 说明。新建工程 project.create 必须首项且新 ID 与当前不同；原工程保留。project.load 必须首项，需要从保存工程列表读取的 expectedSavedRevision；project.delete/restore 必须单独计划，删除移入可恢复归档，恢复不会加载或启动设备。没有保存工程列表和版本时不得猜测这些操作。project.revert 必须单独计划，planId 必须来自已应用的当前工程修改记录；恢复该操作之前的工程配置，覆盖范围通过预览检查，自动控制暂停，不撤销现场设备指令。不得猜测历史计划 ID。
