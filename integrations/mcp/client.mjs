...
export function createApi({base,agentId='mcp-agent',fetchImpl=fetch,timeoutMs=30000}={}){
 ...
 return async(route,body,signal)=>{
  ...
  try{
   const res=await fetchImpl(url,{method:body===undefined?'GET':'POST',headers:{'Content-Type':'application/json','X-FlexHMI-Agent':agentId},...(body===undefined?{}:{body:JSON.stringify(body)}),redirect:'error',signal:combined});
