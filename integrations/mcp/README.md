...
4. 重新连接客户端。应发现 21 个工具（完整范围）、4 个资源与 `build_system` 提示模板。
...
工程修改遵循读取 state/schema → preview → 检查所有关联影响和诊断 → apply。应用不会自动启动控制。工程列表目前仅可读取；尚未开放保存工程的加载、删除工具。高级协议依然通过工程师模式配置。
...
操作范围只约束此 MCP 进程，不是本机 HTTP 的多用户权限边界。Agent ID 是审计标签，不是经过认证的身份。当前仍缺少完整权限、崩溃恢复、通用状态机及真实模型/现场设备验收。本扩展的协议回归通过不等于这些产品能力完成。
