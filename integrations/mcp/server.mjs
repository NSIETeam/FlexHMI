...
保存工程用 projects/project 读取列表、版本与内容。project.load 必须首项；project.delete/restore 单独计划，必须携带 expectedSavedRevision。删除保留归档，恢复不切换工程；当前工程不能删除。加载后不会恢复自动控制授权。计划中断后先读 plans/plan 与状态，已完成结果只返回历史成功，不重放；interrupted 状态需要根据实际状态重新规划。
