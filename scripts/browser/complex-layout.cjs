...
(async()=>{
 ...
 try{
  ...
  assert.deepEqual(await api('project'),before);assert.ok((await tab.locator('#agent-result').innerText()).includes('共用一段路径'));await tab.screenshot({path:path.join(artifacts,'FlexHMI-多分支优化预览.png')});
  ...
  assert.deepEqual(errors,[]);const result={passed:true,browser:'headless Chrome',isolatedData:true,fixture:'complex-process.json with rectangular text nodes',realModel:false,previewDoesNotApply:true,diagnosticsVisible:true,mainOrderRespectsReturn:true,arrowEndpointsPreserved:true,returnTwoBends:true,otherPageUnchanged:true,persistReload:true,localMoveReroutes:true,undoRestores:true,repeatedOptimizeNoChanges:true,remaining:'Two shared-path warnings and a crossing remain in this fixture; no claim of arbitrary layout quality'};fs.writeFileSync(path.join(artifacts,'complex-browser.json'),JSON.stringify(result,null,2));fs.writeFileSync(path.join(artifacts,'optimized-project.json'),JSON.stringify(optimized,null,2));console.log(JSON.stringify(result,null,2));
