...
try {
 ...
 @{arch=$Arch;platform='Windows';installerExecuted=$true;nativeRuntime=$true;simulation=$true;persistence=$true;singleInstance=$true;editorRendered=$true;runtimeRendered=$true;kioskLaunchRequested=$true;stop=$true;upgrade=$true;uninstall=$true;userDataPreserved=$true;physicalHardwareTested=$false} | ConvertTo-Json | Set-Content (Join-Path $Artifacts 'result.json') -Encoding UTF8
