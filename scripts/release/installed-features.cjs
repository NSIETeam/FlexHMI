...
const password='qualification-owner-password-042';
...
(async()=>{
 ...
 if(phase==='exercise'){
  ...
  const client=new Client({name:'installed-protected-qa',version:'0.4.2'}),transport=new StdioClientTransport({command:process.execPath,args:[path.join(root,'integrations/mcp/server.mjs')],env:{...process.env,FLEXHMI_URL:base,FLEXHMI_ACCESS:'full',FLEXHMI_TOKEN:read.token,FLEXHMI_PHYSICAL_WRITES:'0'},stderr:'pipe'});transport.stderr.on('data',b=>process.stderr.write(b));
  try{await client.connect(transport);assert.notEqual((await client.callTool({name:'flexhmi_state',arguments:{}})).isError,true);const denied=await client.callTool({name:'flexhmi_control_pause',arguments:{}});assert.equal(denied.isError,true);assert.equal(denied.structuredContent.status,403);}finally{await client.close()}
  ...
  fs.writeFileSync(evidenceFile,JSON.stringify({...old,restartVerified:true,protectionPersisted:true,grantMetadataPersisted:true,editorHistoryPersisted:true,manualAfterRestart:true},null,2));
