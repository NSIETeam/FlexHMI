...
function mcpConfiguration({port,base='',workDir,protectedMode=false}={}){
 ...
 return {installed,protectedMode,transport:'stdio',version:'0.3.2',configuration:{mcpServers:{flexhmi:{command:process.execPath,args:[script],env}}},message:installed?'接入扩展已安装。把配置添加到支持 MCP 的 Agent 应用，然后重新连接。':'当前安装未包含接入扩展；源码版可在项目目录运行 npm run setup:mcp 安装。',physicalWrites:false,scopeNote:'默认允许完整工程与模拟控制；真实设备写入需要用户单独配置。启用访问保护后，服务端同时校验令牌权限；未启用时仅按可信本机使用。'};
}
