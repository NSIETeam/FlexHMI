...
function validate(p){
 ...
 const ids=new Map(); const unique=(id,location='工程对象')=>{if(!safeId(id))throw Error('ID 无效：'+String(location).slice(0,260)+'；ID 需为 1–90 位字母、数字、下划线或连字符，且不能使用保留名称');if(ids.has(id))throw Error('ID 重复：'+id+'；冲突位置：'+ids.get(id)+' 与 '+location+'。设备、变量、画面、组件和连接的 ID 必须全工程唯一');ids.set(id,location)};
 for(const d of p.devices){
  unique(d.id,'设备/'+d.id);if(!['sim','ModbusTCP'].includes(d.protocol))throw Error('此协议请在工程师模式配置');
