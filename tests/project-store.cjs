...
test('ordinary saves retain before/after history, skip no-ops, reject conflicts and restore through the same plan API',{timeout:25000},async t=>{
 const {dir,save}=fixture(t),original=validate(project('editorhistory'));save(original);fs.writeFileSync(path.join(dir,'active.json'),JSON.stringify(original));let child=await startChild(dir);t.after(()=>child.stop());
 const state=async()=> (await child.request('/agent/state')).data;let firstId;
 for(let i=0;i<26;i++){const current=await state(),next=structuredClone(current.project);next.name='Editor change '+i;const result=await child.request('/editor-save',{project:next,expectedRevision:current.revision});assert.equal(result.status,200);assert.ok(result.data.historyId);firstId??=result.data.historyId;}
 const current=await state(),noOp=await child.request('/editor-save',{project:current.project,expectedRevision:current.revision});assert.equal(noOp.data.historyId,null);
 assert.equal((await child.request('/editor-save',{project:{...current.project,name:'stale'},expectedRevision:digest(original)})).status,409);
 let cursor,seen=[];do{const rows=(await child.request('/agent/plans?source=editor&projectId=editorhistory&limit=7'+(cursor?'&cursor='+cursor:''))).data;assert.ok(Array.isArray(rows));assert.ok(rows.every(p=>p.source==='editor'&&p.canRevert));seen.push(...rows.map(p=>p.id));cursor=rows.length===7?rows.at(-1).id:null;}while(cursor);
 assert.equal(seen.length,26);assert.equal(new Set(seen).size,26);assert.equal((await child.request('/agent/plans?source=agent')).data.length,0);assert.equal((await child.request('/agent/plans?limit=0')).status,400);assert.equal((await child.request('/agent/plans?cursor=plan_missing')).status,400);
 const detail=(await child.request('/agent/plans/'+firstId)).data;assert.equal(detail.before.name,original.name);assert.equal(detail.project.name,'Editor change 0');assert.equal(detail.changes[0].after.name,'Editor change 0');assert.equal((await child.request('/agent/plans/'+firstId+'/apply',{})).data.replayed,true);
 const revert=(await child.request('/agent/plans',{expectedRevision:current.revision,operations:[{op:'project.revert',planId:firstId}]})).data;assert.equal((await child.request('/agent/plans/'+revert.id+'/apply',{})).status,200);assert.equal((await state()).project.name,original.name);await child.stop();child=await startChild(dir);assert.equal((await child.request('/agent/plans/'+firstId)).data.source,'editor');assert.equal((await state()).project.name,original.name);
});
for(const phase of ['before-save','after-save'])test('ordinary save SIGKILL '+phase+' is reconciled without repeating the save',{timeout:25000},async t=>{
 const {dir,save}=fixture(t),p=validate(project('editorcrash'));save(p);fs.writeFileSync(path.join(dir,'active.json'),JSON.stringify(p));let child=await startChild(dir,phase);t.after(()=>child.stop());
